...
const OrderConfirmation = () => {
    const { orderData, loading, error } = useSelector((state) => state.cart); 
                ...
                <div className="border-t border-b border-gray-200 py-4 mb-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <p className="font-semibold">{orderData?.order?.id}</p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-600">Order Date</p>
                            <p className="font-semibold">{new Date(orderData?.order?.createdAt).toLocaleDateString()}</p>
                        </div>
                        <div>
                ...
                <div className="mb-8">
                    ...
                    {orderData?.orderItems && orderData.orderItems.length > 0 ? (
                        orderData.orderItems.map((product) => (
                            <div key={product.product_id} className="flex items-center py-4 border-b border-gray-200">
                                <img
                                    src={`http://localhost:5000${product.image_url}`}
                                    alt={product.product_id}
                                    className="h-20 w-20 object-cover rounded-lg"
                                    onError={(e) => {
                                        e.target.src = "https://images.unsplash.com/photo-1580870069867-74c57ee1bb07"; // Fallback image
                                    }}
                                />
                                <div className="ml-4 flex-1">
                                    <h3 className="font-medium">Product {product.product_id}</h3> 
                ...
                <div className="bg-gray-50 rounded-lg p-6 mb-8">
                    <div className="space-y-2">
                        <div className="flex justify-between">
                            ...
                            <p className="font-medium">${orderData?.order?.total_price?.toFixed(2)}</p>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="border-t border-gray-200 pt-2 mt-2">
                            <div className="flex justify-between">
                                ...
                                <p className="text-lg font-semibold">${orderData?.order?.total_price?.toFixed(2)}</p>
                            </div>
                        </div>
                    </div>
                </div>
